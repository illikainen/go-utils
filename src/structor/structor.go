...
func Apply(value any, opts *Options) error {
	...
	if v.Kind() == reflect.Ptr && v.Elem().Kind() == reflect.Struct {
		err := apply(v.Elem(), validatorSet, opts)
		if err != nil {
			return err
		}
		...
		for i := 0; i < v.Elem().Len(); i++ {
			err := apply(v.Elem().Index(i), validatorSet, opts)
			if err != nil {
				return err
			}
		}
...
func apply(v reflect.Value, validatorSet *ValidatorSet, opts *Options) error {
	t := v.Type()
	for i := 0; i < v.NumField(); i++ {
		...
		if kind == reflect.Struct {
			err := apply(fieldVal, validatorSet, opts)
			if err != nil {
				return err
			}
		} else if kind == reflect.Ptr && fieldVal.Elem().Kind() == reflect.Struct {
			err := apply(fieldVal.Elem(), validatorSet, opts)
			if err != nil {
				return err
			}
